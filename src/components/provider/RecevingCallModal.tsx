...
const ReceivingCallModal: React.FC<ReceivingCallModalProps> = ({
  ...
  const onAccept = () => {
      socket?.emit("callAccepted", {success, callerId, receiverId, caller:'user'});
      navigate(`/provider/voice-call/${callerId}`, {state:true});
      changeModal();
    }
  ...
  return (
    ...
    <div className="w-[90%] md:w-[400px] bg-gray-900 rounded-md p-6 shadow-lg flex flex-col items-center space-y-6">
      ...
      <div className="text-center flex flex-col items-center">
        {/* Caller Image */}
        <img
          src={callerData.image} // Replace 'callerImage' with the URL of the caller's profile picture
          alt="Caller"
          className="w-20 h-20 rounded-full border-4 border-gray-700 shadow-md"
        />
        {/* Caller Name */}
        <h3 className="text-lg font-semibold text-white mt-4">{callerData.name}</h3>
